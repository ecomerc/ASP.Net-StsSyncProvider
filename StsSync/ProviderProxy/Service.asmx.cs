...
namespace ProviderProxy
{
    ...
    public class WebService1 : System.Web.Services.WebService,IListsSoap
    {
        ...
        public System.Xml.XmlNode GetList(string listName)
        {
            ...
            ProviderManager pm = new ProviderManager();
            Provider prov = pm.GetProvider(listId);
        ...
        public System.Xml.XmlNode GetListItemChangesSinceToken(string listName, string viewName, System.Xml.XmlNode query, System.Xml.XmlNode viewFields, string rowLimit, System.Xml.XmlNode queryOptions, string changeToken, System.Xml.XmlNode contains)
        {
            ...
            Guid listId = new Guid(listName); //we'll just crash if it's not a Guid
            Provider prov = pm.GetProvider(listId);
            IProvider iProv = (IProvider)prov.GetIProvider();
            ChangeKey changeKey = null; 
            if (changeToken != null) changeKey = new ChangeKey(changeToken);
            DataSet changes = iProv.GetUpdatesSinceToken(listId, changeKey);
     ...
     xmlns:z='#RowsetSchema'>
            */
            ...
            xelChanges.SetAttribute("LastChangeToken", new ChangeKey(1, 3, prov.ID, DateTime.Now, 1).ToString());
        ...
        public System.Xml.XmlNode UpdateListItems(string listName, System.Xml.XmlNode updates)
        {
            ...
            Provider prov = pm.GetProvider(listId);
            IProvider iProv = (IProvider)prov.GetIProvider();
            ListType listType = iProv.GetProviderType(prov.ID);
            StringDictionary fieldListMapping = iProv.GetFieldMappingsForListType(prov.ID, listType);
            ...
            DataSet emptyDS = iProv.GetEmptyDataSet(prov.ID);
            ...
            foreach (XmlElement xelMethod in xnlMethods)
            {
                ...
                switch (sMethod.ToLower())
                {
                   ...
                   case "new":
                         ...
                         {
                             updateDR = iProv.GetSingleRow(prov.ID, id);
                         }
                         else //new
                         {
                            DataTable dt = iProv.GetEmptyDataSet(prov.ID).Tables[0];
                            updateDR = dt.NewRow();
                        ...
                        if (updateDR != null)
                        {
                            foreach (XmlElement updateField in xnlFields)
                            {
                                ...
                                if (emptyDT.Columns.Contains(fieldName))
                                {
                                    if (fieldDictionary[wssName].ReadOnly) continue; //don't update readonly fields
                                    Type typeOfColumn = emptyDT.Columns[fieldName].DataType;
                                    object fieldValue = DataSetMapper.GetTypedValue(typeOfColumn, updateField.InnerText);
                            ...
                            if (xelMetaInfo != null)
                            {
                                 updateDR[MetaInfoHelper.GetMetaInfoColName(fieldListMapping)] = xelMetaInfo.OuterXml;
                            }

                        }

                        resultDR = iProv.Update(prov.ID, updateDR);
