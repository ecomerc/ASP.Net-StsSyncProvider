...
namespace ProviderProxy
{
    public static class ListsHelper
    {
        ...
        public static XmlElement DataSetToDataNode(XmlElement parentElement, DataSet wssDS, ListType listType,  string requestThread)
        {
            return DataSetToDataNode(parentElement, wssDS, listType, Int32.MaxValue,0,requestThread );
        }

        public static XmlElement DataSetToDataNode(XmlElement parentElement, DataSet wssDS,ListType listType,  int rowLimit,int startRow,string requestThread)
        {
            ...
            DataRowCollection rows = wssDS.Tables[listType.ToString()].Rows;
            
            int rowCount = Math.Min(MAX_ROWS, rowLimit); //ensure we don't go over hardcoded limit
            rowCount = Math.Min(rows.Count, rowCount); //don't go over the number of rows in the set
            ...
            for(;i<rowCount;i++)
            {
                DataRow dr = rows[i+startRow];
                XmlElement zRow = CreateZRow(dataNode, fieldList, null, dr);
                dataNode.AppendChild(zRow);
            }
            if ((i + startRow) < (rows.Count))
            {
        ...
        public static XmlElement CreateZRow(XmlElement dataNode, List<Field> fieldList,StringDictionary fieldMappings, DataRow dr)
        {
            ...
            foreach (Field fld in fieldList)
            {
                ...
                if (fieldName == "MetaInfo") continue;
        ...
        internal static void FillListAttributes(string listName, System.Xml.XmlElement listElement, Provider prov, string appPath)
        {
            ...
            switch (iProv.GetProviderType(prov.ID))
            {
                case ListType.Contacts:
